...
| eth0 IPv4 | `10.1.90.224/23` (static DHCP) |
| eth0 IPv6 | `2404:e80:a137:190::224` (DUID-LL) |
| wlan0 MAC | `dc:a6:32:b4:5f:2a` (onboard BCM43455) |
| wlan0 IPv4 | `10.1.90.225` (static DHCP, ansells-iot) — *not associated as of 2026-08-24* |
| wlan0 IPv6 | `2404:e80:a137:190::225` (DUID-LL) |
| wlanE MAC | `64:70:02:0c:68:02` (RTL8188CUS USB, unmanaged — for AP/client) |
| DNS (eth0) | `eth0.rpi4-esp.iot.welland.mithis.com` (dual-stack), `ipv4.eth0.…` / `ipv6.eth0.…` |
| DNS (wlan0) | `wlan0.rpi4-esp.iot.welland.mithis.com` (dual-stack), `ipv4.wlan0.…` / `ipv6.wlan0.…` |
| PoE Switch | GSM7252PS-S1 (`10.1.5.22`) |
| Switch Port | `1/0/27` (VLAN 90, PoE) |
| SSH | `ssh eth0.rpi4-esp.iot.welland.mithis.com` (user `tim`; this is the default host in `tools/capture_hardware_baseline.py`) |
...
(`/etc/systemd/network/`): `10-eth0.link`, `10-wlan0.link` and `10-wlanE.link` are
the ansible-managed fleet-wide set; the older hand-written `10-wlan0-onboard.link`
and `10-wlanE-usb-rtl8188.link` are still present and match the same MACs.
Because the USB adapter can transiently grab the kernel default name `wlan0` at
boot and make the onboard rename fail with EEXIST, udev rules
`10-net-rename-retry.rules` / `10-wlan0-rename-retry.rules` re-trigger wlan
devices whenever something is renamed away from `wlan0`.

DHCP reservations and DNS records are generated by gdoc2netcfg from the network
Google Sheet (`out/internal/rpi4-esp.conf`).
...
### Using as Access Point (NetworkManager profile)

A NetworkManager AP profile `rpi4-esp-ap` also exists (SSID `rpi4-esp-ap`, 2.4 GHz,
WPA-PSK, `ipv4.method=shared` with `192.168.42.1/24`, autoconnect off). Because
`wlanE` is marked unmanaged, the interface has to be handed back to NetworkManager
before the profile can be activated — the sequence below is the expected one but
has not been exercised yet (untested as of 2026-08-24):

```bash
sudo nmcli dev set wlanE managed yes
sudo nmcli con up rpi4-esp-ap
# ... and to return it to hostapd/manual use:
sudo nmcli con down rpi4-esp-ap
sudo nmcli dev set wlanE managed no
```
...
|---------|----------|---------|
| `lldpd` 1.0.18 | system | LLDP neighbor discovery |
| `uhubctl` 2.6.0 | `/usr/sbin/uhubctl` (needs `sudo`) | USB hub power control |
| `hostapd` 2.10 | system (masked) | WiFi AP for wlanE |
| `openocd` 0.12.0 | `/usr/bin/openocd` | JTAG debugging (ESP32-C3 built-in USB-JTAG) |
...
| `python3-serial` 3.5 | system | pyserial for scripted UART capture |
| `python3` | system | Scripting |

ESP-IDF and the nRF Connect SDK are **not** installed on the station; firmware is
built on a development machine and the binaries copied over (this is what
`tools/capture_hardware_baseline.py` in renode-espemu does).
...
- [x] Readdressed to `10.1.90.224` (eth0) / `10.1.90.225` (wlan0); ESP32-C3 board replaced (MAC now `e8:3d:c1:8c:5c:ac`) — verified 2026-08-24
...
- [x] Automated flash + UART capture scripts: `tools/capture_hardware_baseline.py` / `tools/capture_all_baselines.py` in renode-espemu
